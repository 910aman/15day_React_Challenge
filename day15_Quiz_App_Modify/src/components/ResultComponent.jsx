...
const ResultComponent = ({ filteredItems, result, handleTryAgain, resultName, setResultName, handleSave, score, showScore, highScore, category, selectedAnswers }) => {
  ...
  return (
    <>
      <section className='h-fit max-w-3xl px-2 w-full shadow-3xl cursor-pointer py-3 text-blue-700 flex gap-16 '>
        <div className='flex flex-col gap-5 '>
          ...
          {!showScore == false &&
            <div className='flex items-center flex-col gap-5'>
              ...
              <PDFDownloadLink onClick={LoadingPdf} document={<ReacttoPdf category={category} score={score} result={result} items={filteredItems} selectedAnswers={selectedAnswers} />} fileName='QuizApp'>
                <button type='submit'
                  ...
                  {downloadNow ? "Downloading...": "Download Now"}
                </button>
              </PDFDownloadLink>
            </div>
          }
